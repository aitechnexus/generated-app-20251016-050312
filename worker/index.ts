...
const app = new Hono<{ Bindings: Env; Variables: { user: UserIdentity } }>();
app.use('*', logger());
app.use('/api/*', cors({ origin: '*', allowMethods: ['GET', 'POST', 'PUT', 'DELETE', 'OPTIONS'], allowHeaders: ['Content-Type', 'Authorization', 'cf-access-jwt-assertion'] }));
userRoutes(app);
